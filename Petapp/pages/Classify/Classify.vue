<template>
	<view class="page-body" :style="'height:' + height + 'px'">
		...
		<scroll-view class="nav-right" scroll-y :scroll-top="scrollTop" @scroll="scroll" :style="'height:' + height + 'px'" scroll-with-animation>
			<view v-for="(pets, index) in classifyData" :key="index" class="box">
				<view :id="i == 0 ? 'first' : ''" class="nav-right-item" v-for="(item, i) in pets.pets" :key="i" @click="cart(item.name)">
					<!-- 点击图片跳至详情页面	 -->
					<image :src="`http://localhost:3000/upload/images/${item.imageName[0]}`" style="width: 120rpx;height: 120rpx;" @click="navToPetDetails(item._id)" />
...
export default {
	...
	methods: {
		...mapActions(['classGetpet']),
		//点击图片跳到详情页面(并且传递动物的id值)
		navToPetDetails(_id) {
			uni.navigateTo({
			
				url: '/pages/PetDetails/PetDetails?pet_id='+_id
			});
		},
		async getclassGetpet() {
			const a = await this.classGetpet();
			this.classifyData = a[1].data;
